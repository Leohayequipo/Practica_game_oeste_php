(function() {
  $(function() {
    var $document, death, enterDuelMode, enterOutro, noHonor, reset, resetMessages, restart, showDuelInfo, startButtonOS, win;
    $document = $(document);
    this.$body = $document.find("body");
    
    // opening screen -----------------------------
    this.$openingScreen = $document.find(".js-opening");
    this.$playScreen = $document.find(".js-play");
    this.$startButtonOS = this.$openingScreen.find(".js-start");
    this.$messageOS = this.$openingScreen.find(".js-message");
    this.$playerNameForm = this.$openingScreen.find(".player-name-form");
    this.$playerNameInput = this.$openingScreen.find("#playerName");
    this.$savePlayerBtn = this.$openingScreen.find("#savePlayerBtn");
    
    // Variables para el sistema de puntaje
    this.playerName = "";
    this.currentScore = 0;
    this.levelTimes = [0, 0, 0]; // Tiempos por nivel
    this.honorMaintained = true;
    this.perfectGame = true;
    
    // Función para resetear el formulario del nombre
    this.resetPlayerForm = () => {
      this.$playerNameInput.val('');
      this.playerName = "";
      this.currentScore = 0;
      this.levelTimes = [0, 0, 0];
      this.honorMaintained = true;
      this.perfectGame = true;
      this.$playerNameForm.removeClass("is-hidden");
      this.$startButtonOS.addClass("is-hidden");
    };
    
    // Asegurar que el formulario esté visible al cargar la página
    this.resetPlayerForm();
    
    // Función para calcular puntuación
    this.calculateScore = (level, time) => {
      let baseScore = 0;
      let multiplier = 1;
      
      // Puntos base por nivel
      switch(level) {
        case 1: baseScore = 100; break;
        case 2: baseScore = 250; break;
        case 3: baseScore = 500; break;
      }
      
      // Multiplicador por velocidad
      if (level === 1 && time < 0.5) multiplier = 2;
      else if (level === 1 && time < 1.0) multiplier = 1.5;
      else if (level === 2 && time < 0.3) multiplier = 2;
      else if (level === 2 && time < 0.5) multiplier = 1.5;
      else if (level === 3 && time < 0.2) multiplier = 3;
      else if (level === 3 && time < 0.3) multiplier = 2;
      
      // Bonificación por honor
      if (this.honorMaintained) baseScore += 50;
      
      return Math.round(baseScore * multiplier);
    };
    
    // Función para guardar la puntuación en la base de datos
    this.saveScore = (finalScore) => {
      // Calcular tiempo total sumando todos los niveles
      const totalTime = this.levelTimes.reduce((sum, time) => sum + time, 0);
      
      const scoreData = {
        playerName: this.playerName,
        score: finalScore,
        levelReached: this.level - 1,
        totalTime: totalTime,
        perfectGame: this.perfectGame,
        honorMaintained: this.honorMaintained
      };
      
      console.log('Enviando puntuación:', scoreData); // Debug
      
      fetch('save_score.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(scoreData)
      })
      .then(response => response.json())
      .then(data => {
        if (data.success) {
          if (data.newHighScore) {
            alert(`¡Nueva puntuación más alta! ${finalScore} puntos`);
          }
          console.log('Puntuación guardada exitosamente:', data);
        } else {
          console.error('Error al guardar puntuación:', data.message);
        }
      })
      .catch(error => {
        console.error('Error de conexión:', error);
      });
    };
    
    // Función para guardar el nombre del jugador
    this.savePlayerName = () => {
      const playerName = this.$playerNameInput.val().trim();
      
      if (!playerName) {
        alert("Por favor, ingresa tu nombre para continuar.");
        return;
      }
      
      // Simplemente guardar el nombre localmente y continuar
      this.playerName = playerName;
      this.$playerNameForm.addClass("is-hidden");
      this.$startButtonOS.removeClass("is-hidden");
    };
    
    // Evento para el botón de guardar
    this.$savePlayerBtn.on("click", () => {
      this.savePlayerName();
    });
    
    // Evento submit del formulario
    this.$playerNameForm.on("submit", (e) => {
      e.preventDefault();
      this.savePlayerName();
    });
    
    // Evento para Enter en el input
    this.$playerNameInput.on("keydown", (e) => {
      if (e.key === "Enter" || e.keyCode === 13) {
        e.preventDefault(); // Prevenir comportamiento por defecto
        this.savePlayerName();
      }
    });
    
    // Evento adicional para mayor compatibilidad
    this.$playerNameInput.on("keyup", (e) => {
      if (e.key === "Enter" || e.keyCode === 13) {
        this.savePlayerName();
      }
    });
    
    startButtonOS = () => {
      this.$startButtonOS.addClass("is-hidden");
      this.$messageOS.removeClass("is-hidden");
      return this.$openingScreen.addClass("has-message");
    };
    
    this.$startButtonOS.on("click", () => {
      return startButtonOS();
    });
    this.$continueButtonOS = this.$openingScreen.find(".js-continue");
    this.$continueButtonOS.on("click", () => {
      this.$openingScreen.addClass("is-hidden");
      this.$playScreen.removeClass("is-hidden");
      this.$openingScreen.removeClass("has-message");
      return showDuelInfo();
    });
    
    // play screen -----------------------------  
    this.$messagePS = this.$playScreen.find(".js-message");
    this.$duelButton = $document.find(".js-duel");
    this.$drawButton = $document.find(".js-draw");
    this.$sheriff = $document.find(".js-sheriff");
    this.$opponent = $document.find(".js-opponent");
    this.$resultPS = $document.find(".js-result");
    this.result;
    this.$timer;
    this.dueling = false;
    this.level = 1;
    this.gameOver = false;
    this.noHonor = 1;
    this.totalTime = 0;
    restart = () => {
      // completely restarts game
      this.level = 1;
      this.noHonor = 0;
      this.dueling = false;
      this.gameOver = false;
      this.totalTime = 0;
      this.currentScore = 0; // Resetear puntuación a 0
      this.levelTimes = [0, 0, 0]; // Resetear tiempos
      this.honorMaintained = true; // Resetear honor
      this.perfectGame = true; // Resetear juego perfecto
      resetMessages();
      this.$outroScreen.addClass("is-hidden");
      this.$playScreen.addClass("is-hidden");
      this.$openingScreen.removeClass("is-hidden");
      
      // Siempre mostrar el formulario del nombre al reiniciar
      this.$playerNameForm.removeClass("is-hidden");
      this.$startButtonOS.addClass("is-hidden");
      this.$messageOS.addClass("is-hidden");
      this.$playerNameInput.val(''); // Limpiar el input
      this.playerName = ""; // Resetear el nombre del jugador
      
      this.$body.removeClass("game-over");
      this.$body.removeClass("game-end");
      this.$sheriff.removeClass("is-dead");
      return this.$sheriff.removeClass("is-gone");
    };
    death = () => {
      // Guardar puntuación aunque hayas perdido
      this.saveScore(this.currentScore);
      
      this.$opponent.addClass("is-shooting");
      this.$sheriff.addClass("is-dead");
      return setTimeout((() => {
        this.$sheriff.addClass("is-gone");
        this.$opponent.removeClass("is-shooting");
        this.$resultPS.html(`Fin del Juego - Puntuación: ${this.currentScore}`);
        this.$duelButton.removeClass("is-hidden");
        this.level = 1;
        this.gameOver = true;
        this.$body.addClass("game-over");
      }), 200);
    };
    win = () => {
      // Guardar tiempo del nivel actual
      this.levelTimes[this.level - 1] = this.result;
      
      // Calcular puntuación del nivel
      const levelScore = this.calculateScore(this.level, this.result);
      this.currentScore += levelScore;
      
      this.$sheriff.addClass("is-shooting");
      this.$opponent.addClass("is-dead");
      this.$resultPS.html(`${this.playerName}, justicia servida en ${this.result.toFixed(3)} segundos - Puntos: ${levelScore} (Total: ${this.currentScore})`);
      this.level++;
      
      // Si llegaste al nivel 3, guardar la puntuación final
      if (this.level === 4) {
        this.saveScore(this.currentScore);
      }
      
      this.$resultContinue.removeClass("is-hidden");
      setTimeout((() => {
        this.$sheriff.removeClass("is-shooting");
        this.$sheriff.addClass("is-armed");
      }), 200);
      return setTimeout((() => {
        this.$sheriff.removeClass("is-armed");
      }), 1000);
    };
    this.$resultContinue = $document.find(".js-result-continue");
    this.$resultContinue.on("click", () => {
      this.$resultContinue.addClass("is-hidden");
      resetMessages();
      // if won the game
      if (this.level === 4) {
        return enterOutro();
      } else {
        // continue as normal
        return showDuelInfo();
      }
    });
    reset = () => {
      // stops game if no reaction
      // Guardar puntuación antes de llamar a death
      this.saveScore(this.currentScore);
      
      death();
      clearInterval(this.$timer);
      this.$duelButton.removeClass("is-hidden");
      return this.$drawButton.addClass("is-hidden");
    };
    showDuelInfo = () => {
      this.$opponent.removeClass("is-armed");
      this.$opponent.removeClass("is-dead");
      this.$opponent.removeClass("opponent--1");
      this.$opponent.removeClass("opponent--2");
      this.$opponent.removeClass("opponent--3");
      this.$opponent.addClass("opponent--" + this.level);
      switch (this.level) {
        case 3:
          this.$messagePS.html("Big Gilly Boulder anhela el trabajo de azotar a un mocoso como tú. ¡Es un artista hábil con el revólver! Ten cuidado");
          break;
        case 2:
          this.$messagePS.html("Me llamo Goldie Gaia. Dirijo el salón aunque tenga que enviarte al cementerio con las botas puestas.");
          break;
        default:
          this.$messagePS.html("Pumpgun Ed no es un hombre con quien meterse. Ya ves, este grandullón te tumba en el poste y te azota hasta hacerte trizas");
      }
      return this.$duelButton.removeClass("is-hidden");
    };
    resetMessages = () => {
      this.$messagePS.html("");
      return this.$resultPS.html("");
    };
    enterDuelMode = () => {
      this.$duelButton.addClass("is-hidden");
      this.$drawButton.removeClass("is-hidden");
      resetMessages();
      this.delay = (Math.floor(Math.random() * (6 - 1) + 1)) * 1000;
      // start duel
      return this.$duel = setTimeout((() => {
        var startTime;
        this.dueling = true;
        // style start
        this.$opponent.addClass("is-armed");
        // start timer
        startTime = Date.now();
        return this.$timer = setInterval((() => {
          var elapsedTime;
          elapsedTime = Date.now() - startTime;
          // announce result
          this.result = elapsedTime / 1000;
          if (this.result > 2) {
            reset();
          }
        }), 10);
      }), this.delay);
    };
    noHonor = () => {
      var noHonorMessage, random;
      // if clicked to early
      this.honorMaintained = false; // Marcar que se perdió el honor
      clearInterval(this.$duel);
      this.$duelButton.removeClass("is-hidden");
      this.$drawButton.addClass("is-hidden");
      switch (this.noHonor) {
        case 1:
          noHonorMessage = "No quieres ser demasiado precipitado con el trío. Ellos tienen que desenfundar primero.";
          this.noHonor++;
          break;
        case 2:
          noHonorMessage = "Seguro que estás jugando con suerte. Mejor ve y arregla tus cobijas.";
          this.noHonor++;
          break;
        default:
          random = Math.floor(Math.random() * (4 - 1)) + 1;
          switch (random) {
            case 1:
              noHonorMessage = "¡Mezclándolo con algo de suerte! ¡Y a esta hora del día!";
              break;
            case 2:
              noHonorMessage = "Sucedió algo repentino, ¿verdad?";
              break;
            default:
              noHonorMessage = "¡Qué demonios! ¿Qué clase de cosa muestra tan poco honor en un duelo rápido?";
          }
      }
      return this.$resultPS.html(noHonorMessage);
    };
    
    // Duel button
    this.$duelButton.on("click", () => {
      if (this.gameOver) {
        return restart();
      } else {
        return enterDuelMode();
      }
    });
    // Draw button
    this.$drawButton.on("click", () => {
      if (this.dueling) {
        // good draw
        clearInterval(this.$timer);
        this.$opponent.removeClass("is-armed");
        this.$drawButton.addClass("is-hidden");
        switch (this.level) {
          case 3:
            if (this.result < 0.33) {
              win();
            } else {
              death();
            }
            break;
          case 2:
            if (this.result < 0.4) {
              win();
            } else {
              death();
            }
            break;
          default:
            if (this.result < 2) {
              win();
            } else {
              death();
            }
        }
      } else {
        // failed draw
        noHonor();
      }
      return this.dueling = false;
    });
    
    // Outro screen -----------------------------  
    this.$outroScreen = $document.find(".js-outro");
    this.$saveAndQuitButton = this.$outroScreen.find(".js-save-and-quit");
    this.$totalTime = this.$outroScreen.find(".js-total-time");
    enterOutro = () => {
      // Calcular tiempo total sumando todos los niveles
      const totalTime = this.levelTimes.reduce((sum, time) => sum + time, 0);
      this.$totalTime.html(`Tiempo total: ${totalTime.toFixed(3)} segundos`);
      
      // Calcular puntuación final
      const finalScore = this.currentScore;
      
      // Personalizar el mensaje final con el nombre del jugador
      this.$outroScreen.find(".js-final-message").html(
        `¡Demonios, ${this.playerName}! Eres rápido con el revólver. ¡Serás un sheriff de primera que se come el infierno! Puntuación final: ${finalScore}`
      );
      
      this.$playScreen.addClass("is-hidden");
      this.$outroScreen.removeClass("is-hidden");
      this.$body.addClass("game-end");
      return this.$totalTime = 0;
    };
    return this.$saveAndQuitButton.on("click", () => {
      return restart();
    });
  });

}).call(this);

//# sourceMappingURL=data:application/json;base64,
//# sourceURL=coffeescript